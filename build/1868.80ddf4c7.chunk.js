...
`,C={common:_t,light:y,dark:d,additional:_},J=()=>{const b=localStorage.getItem("STRAPI_THEME")||"light",{theme:P,themeOverwrite:a}=globalThis.CKEditorConfig||{},r=a?P:{...C,...P};return(0,X.Ud)`
       ${r.common}
       ${r[b]}
       ${r.additional}
   `},z=({isOpen:b,onChange:P,onToggle:a,editor:r,uploadConfig:{responsiveDimensions:t}})=>{const{components:e}=(0,O.um)(),o=e["media-library"],i=c=>{let n="";c.map(({name:p,url:v,alt:D,formats:V,mime:Z,width:Q,height:L})=>{if(Z.includes("image"))if(V&&t){let H="";Object.keys(V).sort((g,m)=>V[g].width-V[m].width).map(g=>H+=(0,O.AZ)(V[g].url)+` ${V[g].width}w,`),n+=`<img src="${v}" alt="${D}" width="${Q}" height="${L}" srcset="${H}" />`}else n+=`<img src="${v}" alt="${D}" width="${Q}" height="${L}" />`;else Z.includes("application/pdf")?n=`<a href="${(0,O.AZ)(v)}" download="${p}">${p||"Download PDF"}</a>`:Z.includes("video")&&(n=`
            <video class="video" controls width="500px">
                <source src="${(0,O.AZ)(v)}" type="${Z}" />
            <video/>`)});const h=r.data.processor.toView(n),l=r.data.toModel(h);r.model.insertContent(l),a()},s=c=>{const n=c.map(h=>({name:h.name,alt:h.alternativeText||h.name,url:(0,O.AZ)(h.url),mime:h.mime,formats:h.formats,width:h.width,height:h.height}));i(n)};return b?(0,I.jsx)(o,{onClose:a,onSelectAssets:s}):null};z.defaultProps={isOpen:!1,onChange:()=>{},onToggle:()=>{}},z.propTypes={isOpen:B().bool,onChange:B().func,onToggle:B().func};const ot=z;var tt=x(76368);/*!
